# =============================================================================
# FINAL ENHANCED HTML PRESENTATION
# This script creates the final enhanced HTML presentation incorporating all
# the scientific improvements, enhanced statistics, and comprehensive analysis.
# =============================================================================
...
cat("=== FINAL ENHANCED HTML PRESENTATION ===\n")
cat("Creating comprehensive enhanced presentation with all improvements\n\n")

# Create output directory for the final enhanced presentation
final_output_dir <- "final_enhanced_presentation"
final_figures_dir <- file.path(final_output_dir, "figures")

if (!dir.exists(final_output_dir)) {
  dir.create(final_output_dir, recursive = TRUE)
}
if (!dir.exists(final_figures_dir)) {
  dir.create(final_figures_dir, recursive = TRUE)
}
...
file.copy("custom.css", file.path(final_output_dir, "custom.css"), overwrite = TRUE)

cat("Copying all figures...\n")
copied_files_count <- 0

# Copy individual PDF figures
for (fig_name in individual_pdf_figures) {
  src_path <- file.path(getwd(), fig_name)
  dest_path <- file.path(final_figures_dir, basename(fig_name))
...
for (dir in figure_dirs) {
  ...
  if (dir.exists(full_dir_path)) {
    ...
    for (fig_path in figures_in_dir) {
      dest_path <- file.path(final_figures_dir, basename(fig_path))
...
all_samples_t <- as.data.frame(t(read.table(file.path(metadata_dir, "GSE168714_All_samples_enrolment.txt"),
                                            header = TRUE, sep = "\t", stringsAsFactors = FALSE, row.names = 1)))
all_samples_t$sample <- rownames(all_samples_t)
if ("Riluzile" %in% names(all_samples_t)) {
  all_samples_t <- all_samples_t %>% rename(Riluzole = Riluzile)
}
...
rmd_file_path <- file.path(final_output_dir, "final_enhanced_presentation.Rmd")
writeLines(rmd_content, rmd_file_path)

# Render the HTML presentation
cat("Rendering final enhanced HTML presentation...\n")
tryCatch({
  render(rmd_file_path, 
         output_file = "final_enhanced_presentation.html",
         output_dir = final_output_dir,
         quiet = FALSE)
  cat("✅ Final enhanced HTML presentation rendered successfully!\n")
}, error = function(e) {
  cat("❌ Error rendering HTML presentation:", e$message, "\n")
  cat("This might be due to missing data files or figure paths.\n")
})

cat("\n=== FINAL ENHANCED HTML PRESENTATION COMPLETED ===\n")
cat("📁 Output directory:", final_output_dir, "\n")
cat("📊 Total figures copied:", copied_files_count, "\n")
cat("📄 R Markdown file:", rmd_file_path, "\n")
cat("🌐 HTML file:", file.path(final_output_dir, "final_enhanced_presentation.html"), "\n")
...
cat("The final enhanced presentation includes:\n")
cat("- Comprehensive statistical validation with effect sizes and power analysis\n")
cat("- Robustness testing through bootstrap validation and sensitivity analysis\n")
cat("- Mechanistic interpretation with biological plausibility assessment\n")
cat("- Comparative analysis with other neurodegenerative diseases\n")
cat("- Clinical implications and therapeutic target identification\n")
cat("- Integration with original paper findings (GSE168714)\n")
cat("- All original figures and analyses with enhanced interpretation\n")
