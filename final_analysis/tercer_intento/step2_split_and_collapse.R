...
cat("=== STEP 2: SPLIT AND COLLAPSE MUTATIONS ===\n")
...
split_mutations <- function(data) {
  cat("  - Processing single mutations first...\n")
  split_data <- data[!multi_mut_rows, ]  # Start with single mutations
  cat("  - Single mutations:", nrow(split_data), "\n")
...
cat("Analyzing mutation types after split/collapse...\n")

mutation_types_after <- str_extract(collapse_data$`pos:mut`, "[A-Z]{2}$")
mutation_types_after <- mutation_types_after[!is.na(mutation_types_after)]

if (length(mutation_types_after) > 0) {
  mutation_counts_after <- table(mutation_types_after)
  cat("📊 Mutation types after split/collapse:\n")
  print(mutation_counts_after)
  
  # Create mutation type comparison
  mutation_df_after <- data.frame(
    mutation_type = names(mutation_counts_after),
    count = as.numeric(mutation_counts_after)
  )
  
  p3 <- ggplot(mutation_df_after, aes(x = reorder(mutation_type, count), y = count, fill = mutation_type)) +
    geom_bar(stat = "identity", alpha = 0.8, color = "black", linewidth = 0.3) +
    scale_fill_brewer(palette = "Set3") +
    labs(title = "Mutation Types After Split and Collapse",
         x = "Mutation Type", 
         y = "Count") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5, size = 18, face = "bold"),
  ...
  ggsave(file.path(figures_dir, "mutation_types_after.png"), p3, 
         width = 12, height = 8, dpi = 300)
}
