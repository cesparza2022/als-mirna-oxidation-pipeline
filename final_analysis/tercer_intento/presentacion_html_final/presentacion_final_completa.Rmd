...
```{r oxidative-outliers, fig.cap='Análisis de outliers en la carga oxidativa.'}
...
## Análisis de Pathways y Redes de miRNAs

### miRNAs Contributivos y Familias
- Se identificaron miRNAs clave y sus familias que muestran patrones diferenciales.
- Esto proporciona una base para la interpretación biológica y funcional.

```{r pathways-family-contributions, fig.cap='Heatmap de contribuciones de familias de miRNAs a la diferenciación.'}
...
```{r pathways-critical-positions, fig.cap='Análisis de posiciones críticas en miRNAs.'}
knitr::include_graphics(file.path(base_path, 'figures_pathways/02_critical_positions_analysis.png'))
```

### Redes de Correlación de miRNAs
- Se construyeron redes de correlación para visualizar las interacciones entre miRNAs.
- La identificación de módulos o comunidades en la red puede revelar vías biológicas alteradas.

```{r pathways-network, fig.cap='Red de correlación de miRNAs, mostrando interacciones y comunidades.'}
...
### Análisis de Metadatos del Paper Original
```{r metadata-analysis, fig.cap='Análisis de metadatos del paper original GSE168714.'}
knitr::include_graphics(file.path(base_path, 'metadata_analysis/figures/01_batch_distribution.png'))
```

### Distribución por Sexo y Tipo de Onset
```{r metadata-sex-onset, fig.cap='Distribución por sexo y tipo de onset en el dataset original.'}
knitr::include_graphics(file.path(base_path, 'metadata_analysis/figures/02_sex_distribution.png'))
```

### Correlación miR-181 vs NfL
```{r metadata-correlation, fig.cap='Correlación entre miR-181 y NfL en el dataset original.'}
...
### Desarrollo de Score Diagnóstico
- Se desarrolló un score diagnóstico basado en SNVs oxidativos.
- El score combina múltiples métricas de carga oxidativa.

```{r clinical-score, fig.cap='Desarrollo del score diagnóstico basado en SNVs oxidativos.'}
...
```{r clinical-sensitivity, fig.cap='Análisis de sensibilidad y especificidad del score diagnóstico.'}
...
```{r clinical-correlation, fig.cap='Correlación del score con variables clínicas.'}
