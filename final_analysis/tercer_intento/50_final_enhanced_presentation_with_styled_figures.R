# =============================================================================
# FINAL ENHANCED PRESENTATION WITH STYLED FIGURES
# This script creates the final presentation with all enhanced styled figures
# =============================================================================
...
cat("=== FINAL ENHANCED PRESENTATION WITH STYLED FIGURES ===\n")
cat("Creating final presentation with all enhanced styled figures\n\n")

# Create output directory for the final presentation
final_dir <- "final_enhanced_presentation_styled"
final_figures_dir <- file.path(final_dir, "figures")
...
# Copy enhanced styled figures
if (dir.exists("enhanced_styled_figures")) {
  enhanced_figures <- list.files("enhanced_styled_figures", full.names = TRUE)
  ...
  cat("Copied", length(enhanced_figures), "enhanced styled figures\n")
}

# Copy other important figures from existing presentations
figure_sources <- c(
  "ultimate_enhanced_presentation/figures",
  "final_comprehensive_enhanced_presentation/figures",
  "comprehensive_html_presentation_fixed/figures"
)

for (source_dir in figure_sources) {
  if (dir.exists(source_dir)) {
    figures_to_copy <- list.files(source_dir, full.names = TRUE)
    for (fig_path in figures_to_copy) {
      dest_path <- file.path(final_figures_dir, basename(fig_path))
      if (!file.exists(dest_path)) {  # Don't overwrite enhanced figures
        file.copy(fig_path, dest_path, overwrite = TRUE)
      }
    }
    cat("Copied additional figures from", source_dir, "\n")
  }
}

# Copy custom CSS file
if (file.exists("custom.css")) {
  file.copy("custom.css", file.path(final_dir, "custom.css"), overwrite = TRUE)
}

# Create the final enhanced R Markdown content with all data embedded
rmd_content <- c(
  "---",
...
rmd_file_path <- file.path(final_dir, "final_enhanced_presentation_styled.Rmd")
writeLines(rmd_content, rmd_file_path)

# Render the HTML presentation
cat("Rendering final enhanced HTML presentation with styled figures...\n")
tryCatch({
  render(rmd_file_path, 
         output_file = "final_enhanced_presentation_styled.html",
         output_dir = final_dir,
         quiet = FALSE)
  cat("✅ Final enhanced HTML presentation with styled figures rendered successfully!\n")
}, error = function(e) {
  cat("❌ Error rendering HTML presentation:", e$message, "\n")
})

cat("\n=== FINAL ENHANCED PRESENTATION WITH STYLED FIGURES COMPLETED ===\n")
cat("📁 Output directory:", final_dir, "\n")
cat("📄 R Markdown file:", rmd_file_path, "\n")
cat("🌐 HTML file:", file.path(final_dir, "final_enhanced_presentation_styled.html"), "\n")
cat("🎨 Enhanced with professionally styled figures\n")
cat("🔬 Comprehensive statistical validation and critical review\n")
cat("🧬 Detailed mechanistic interpretation and biological plausibility\n")
