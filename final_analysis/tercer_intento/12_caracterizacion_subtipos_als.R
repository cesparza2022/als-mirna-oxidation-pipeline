...
# =============================================================================
# CARACTERIZACIÓN DE SUBTIPOS DE ALS
# =============================================================================
...
calculate_sample_metrics <- function(sample_names, vaf_matrix) {
  ...
  metrics <- data.frame(
    ...
    Mean_VAF = colMeans(sample_data[sample_data > 0], na.rm = TRUE),
    Max_VAF = apply(sample_data, 2, max, na.rm = TRUE),
    Median_VAF = apply(sample_data, 2, function(x) median(x[x > 0], na.rm = TRUE)),
