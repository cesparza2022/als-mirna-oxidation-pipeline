...
# Set paths (relative to step1_5/scripts/)
...
data <- read.csv(file.path(tables_dir, "ALL_MUTATIONS_VAF_FILTERED.csv"), check.names = FALSE)

cat(sprintf("   ✅ Total rows: %s\n", format(nrow(data), big.mark = ",")))
cat(sprintf("   ✅ Columns: %d\n", ncol(data)))

# Load filter report
filter_report <- read.csv(file.path(tables_dir, "vaf_filter_report.csv"))
...
stats_by_type <- read.csv(file.path(tables_dir, "vaf_statistics_by_type.csv"))
...
stats_by_mirna <- read.csv(file.path(tables_dir, "vaf_statistics_by_mirna.csv"))
