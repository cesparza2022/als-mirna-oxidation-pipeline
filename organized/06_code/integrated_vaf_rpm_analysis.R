# Análisis integrado de VAF y RPM con datos procesados
...
cat("🚀 ANÁLISIS INTEGRADO VAF Y RPM\n")
...
cat("\n🔧 Calculando RPM promedio por miRNA...\n")
mean_rpm <- df_processed %>%
  rowwise() %>%
  mutate(
    mean_rpm = mean(
      sapply(total_cols, function(col) {
        total_count <- .data[[col]]
        lib_size <- library_sizes[col]
  ...
  ungroup() %>%
  select(miRNA.name, pos.mut, mean_rpm)
...
seed_gt_mutations <- df_processed %>%
  ...
  left_join(mean_rpm, by = c("miRNA.name", "pos.mut"))
...
vaf_matrix <- vaf_filtered %>%
  filter(miRNA.name %in% top_mirnas$miRNA.name) %>%
  rowwise() %>%
  mutate(
    # Create VAF values for each sample
    vaf_row = list(
      sapply(1:length(snv_cols), function(i) {
        snv_count <- .data[[snv_cols[i]]]
        total_count <- .data[[total_cols[i]]]
  ...
  summarise(
    vaf_values = list(vaf_row),
    .groups = "drop"
  ) %>%
  mutate(
    vaf_matrix = map(vaf_values, ~ matrix(.x, nrow = 1))
  )
...
if (nrow(vaf_filtered) > 0) {
  ...
  # Create heatmap
  png("outputs/figures/integrated_vaf_heatmap.png", width = 1200, height = 800)
  pheatmap(
    vaf_heatmap_data,
    cluster_rows = TRUE,
    cluster_cols = TRUE,
  ...
  cat("   ✅ Heatmap guardado: outputs/figures/integrated_vaf_heatmap.png\n")
}
...
# RPM distribution
png("outputs/figures/integrated_rpm_distribution.png", width = 800, height = 600)
ggplot(mean_rpm, aes(x = mean_rpm)) +
  geom_histogram(bins = 50, fill = "skyblue", alpha = 0.7) +
  scale_x_log10() +
  labs(
    title = "Distribution of Mean RPM per miRNA",
    x = "Mean RPM (log10)",
    y = "Count"
  ) +
  theme_minimal()
...
# VAF distribution
png("outputs/figures/integrated_vaf_distribution.png", width = 800, height = 600)
...
png("outputs/figures/integrated_top_mirnas.png", width = 1000, height = 600)
...
write_tsv(top_mirnas, "outputs/integrated_top_mirnas.tsv")
write_tsv(vaf_filtered, "outputs/integrated_vaf_data.tsv")
write_tsv(sample_annotation, "outputs/integrated_sample_annotation.tsv")

# Create summary report
cat("\n📋 Creando reporte de resumen...\n")
report <- paste0(
  "# ANÁLISIS INTEGRADO VAF Y RPM - REPORTE\n\n",
  "## Resumen del Procesamiento\n",
  "- **Dataset original**: ", nrow(df_processed), " miRNAs\n",
  ...
  "\n## Archivos Generados\n",
  "- `outputs/integrated_top_mirnas.tsv`: Top miRNAs\n",
  "- `outputs/integrated_vaf_data.tsv`: Datos VAF\n",
  "- `outputs/integrated_sample_annotation.tsv`: Anotación de muestras\n",
  "- `outputs/figures/integrated_*`: Gráficos y heatmap\n"
)

writeLines(report, "outputs/integrated_analysis_report.md")

cat("\n🎉 Análisis integrado completado!\n")
cat("   📊 Archivos generados:\n")
cat("   - outputs/integrated_top_mirnas.tsv\n")
cat("   - outputs/integrated_vaf_data.tsv\n")
cat("   - outputs/integrated_sample_annotation.tsv\n")
cat("   - outputs/integrated_analysis_report.md\n")
cat("   - outputs/figures/integrated_*\n")
