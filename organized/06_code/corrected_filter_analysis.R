...
# =============================================================================
# ANÁLISIS COMPREHENSIVO DE FILTROS DE EXPRESIÓN (CORREGIDO)
# =============================================================================
...
main <- function() {
  ...
  # Verificar si hay mutaciones G>T
  gt_mutations <- df %>%
    filter(str_detect(`pos:mut`, "G>T"))
  
  cat("SNVs G>T encontrados:", nrow(gt_mutations), "\n")
  
  if (nrow(gt_mutations) == 0) {
    cat("No se encontraron mutaciones G>T. Verificando otros tipos de mutación...\n")
    
    # Verificar otros tipos de mutación
    mutation_types <- df %>%
      filter(`pos:mut` != "PM") %>%
      count(`pos:mut`, sort = TRUE)
    
    cat("Tipos de mutación encontrados:\n")
    print(head(mutation_types, 10))
    
    # Usar todas las mutaciones para el análisis
    df_mutations <- df %>%
      filter(`pos:mut` != "PM")
    
    cat("Usando todas las mutaciones para el análisis:", nrow(df_mutations), "\n")
  } else {
    df_mutations <- gt_mutations
  }
  ...
  for (threshold in rpm_thresholds) {
    for (method in methods) {
      ...
      # Calcular RPM
      rpm_result <- calculate_rpm(df_mutations, total_cols, method)
      ...
      # Contar mutaciones en región semilla
...
filter_by_rpm_threshold <- function(df, snv_cols, threshold, method = "mean") {
  ...
  # Verificar que las columnas RPM existen
  available_rpm_cols <- intersect(rpm_cols, names(df))
  
  if (length(available_rpm_cols) == 0) {
    cat("No se encontraron columnas RPM. Calculando RPM sobre la marcha...\n")
    
    # Calcular RPM directamente
    if (method == "mean") {
      df$mean_rpm <- rowMeans(select(df, all_of(snv_cols)), na.rm = TRUE)
    } else if (method == "median") {
      df$mean_rpm <- apply(select(df, all_of(snv_cols)), 1, median, na.rm = TRUE)
    } else if (method == "max") {
      df$mean_rpm <- apply(select(df, all_of(snv_cols)), 1, max, na.rm = TRUE)
    }
  } else {
    if (method == "mean") {
      df$mean_rpm <- rowMeans(select(df, all_of(available_rpm_cols)), na.rm = TRUE)
    } else if (method == "median") {
      df$mean_rpm <- apply(select(df, all_of(available_rpm_cols)), 1, median, na.rm = TRUE)
    } else if (method == "max") {
      df$mean_rpm <- apply(select(df, all_of(available_rpm_cols)), 1, max, na.rm = TRUE)
    }
  }
...
generate_comparative_report <- function(results) {
  ...
  report_content <- paste0(
    ...
    "**Fecha:** ", Sys.Date(), "\n\n",
    "## RESUMEN EJECUTIVO\n\n",
    "Este análisis compara diferentes estrategias de filtrado de miRNAs para mutaciones en región semilla.\n\n",
