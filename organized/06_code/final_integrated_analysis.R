# Análisis final integrado de VAF y RPM con datos procesados correctamente
...
cat("🚀 ANÁLISIS FINAL INTEGRADO VAF Y RPM\n")
...
vaf_data <- seed_gt_mutations %>%
  ...
  mutate(
    ...
    vaf_values = list(
      sapply(seq_along(snv_cols), function(i) {
...
rpm_data <- vaf_filtered %>%
  rowwise() %>%
  mutate(
    mean_rpm = mean(
      sapply(seq_along(total_cols), function(i) {
        total_count <- .data[[total_cols[i]]]
        lib_size <- library_sizes[total_cols[i]]
        if (lib_size > 0) (total_count / lib_size) * 1e6 else 0
      }), 
      na.rm = TRUE
    )
  ) %>%
...
if (nrow(vaf_filtered) > 0) {
  ...
  # Create heatmap
  cat("\n🎨 Creando heatmap...\n")
  png("outputs/figures/final_integrated_vaf_heatmap.png", width = 1200, height = 800)
  pheatmap(
    vaf_heatmap_data,
    cluster_rows = TRUE,
    cluster_cols = TRUE,
    ...
    show_rownames = FALSE,
    show_colnames = FALSE,
    main = "VAF Heatmap - G>T Mutations in Seed Region (Final Analysis)"
  )
  dev.off()
  
  cat("   ✅ Heatmap guardado: outputs/figures/final_integrated_vaf_heatmap.png\n")
}
...
# VAF distribution
png("outputs/figures/final_integrated_vaf_distribution.png", width = 800, height = 600)
ggplot(vaf_filtered, aes(x = mean_vaf)) +
  geom_histogram(bins = 50, fill = "lightcoral", alpha = 0.7) +
  labs(
    title = "Distribution of Mean VAF per Mutation (Final Analysis)",
    x = "Mean VAF",
    y = "Count"
  ) +
  theme_minimal()
...
# RPM distribution
png("outputs/figures/final_integrated_rpm_distribution.png", width = 800, height = 600)
ggplot(rpm_data, aes(x = mean_rpm)) +
  geom_histogram(bins = 50, fill = "skyblue", alpha = 0.7) +
  scale_x_log10() +
  labs(
    title = "Distribution of Mean RPM per miRNA (Final Analysis)",
    x = "Mean RPM (log10)",
    y = "Count"
  ) +
  theme_minimal()
...
png("outputs/figures/final_integrated_top_mirnas.png", width = 1000, height = 600)
ggplot(top_mirnas[1:20, ], aes(x = reorder(miRNA.name, total_gt_counts), y = total_gt_counts)) +
  geom_col(fill = "steelblue", alpha = 0.7) +
  coord_flip() +
  labs(
    title = "Top 20 miRNAs by G>T Counts in Seed Region (Final Analysis)",
    x = "miRNA",
    y = "Total G>T Counts"
  ) +
  theme_minimal()
...
png("outputs/figures/final_integrated_position_distribution.png", width = 800, height = 600)
...
write_tsv(top_mirnas, "outputs/final_integrated_top_mirnas.tsv")
write_tsv(vaf_filtered, "outputs/final_integrated_vaf_data.tsv")
write_tsv(sample_annotation, "outputs/final_integrated_sample_annotation.tsv")

# Create summary report
cat("\n📋 Creando reporte de resumen...\n")
report <- paste0(
  "# ANÁLISIS FINAL INTEGRADO VAF Y RPM - REPORTE\n\n",
  ...
  "\n## Archivos Generados\n",
  "- `outputs/final_integrated_top_mirnas.tsv`: Top miRNAs\n",
  "- `outputs/final_integrated_vaf_data.tsv`: Datos VAF\n",
  "- `outputs/final_integrated_sample_annotation.tsv`: Anotación de muestras\n",
  "- `outputs/figures/final_integrated_*`: Gráficos y heatmap\n"
)

writeLines(report, "outputs/final_integrated_analysis_report.md")

cat("\n🎉 Análisis final integrado completado!\n")
cat("   📊 Archivos generados:\n")
cat("   - outputs/final_integrated_top_mirnas.tsv\n")
cat("   - outputs/final_integrated_vaf_data.tsv\n")
cat("   - outputs/final_integrated_sample_annotation.tsv\n")
cat("   - outputs/final_integrated_analysis_report.md\n")
cat("   - outputs/figures/final_integrated_*\n")
