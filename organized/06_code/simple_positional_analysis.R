#' Simple Positional Analysis of miRNA Oxidation
#' 
...
#' Run simple positional analysis
run_simple_positional_analysis <- function() {
  cat("=== SIMPLE POSITIONAL ANALYSIS OF miRNA OXIDATION ===\n")
  ...
  extract_positions <- function(mutation_string) {
    # Handle single mutations like "2:GT"
    if (str_detect(mutation_string, "^\\d+:")) {
      pos <- as.numeric(str_extract(mutation_string, "^\\d+"))
      return(data.frame(position = pos, mutation_type = str_extract(mutation_string, "[A-Z]+$")))
    }
  ...
  cat("\nStatistical Comparison by Position:\n")
...
# Run the analysis
if (!interactive()) {
  results <- run_simple_positional_analysis()
}
