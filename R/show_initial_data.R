...
cat("\n📋 Nombres de columnas (últimas 5):\n")
print(tail(names(df_main), 5))
...
cat("📊 Ejemplo 1 - hsa-let-7i-5p_4_GT:\n")
example1 <- df_main %>%
...
cat("\n📊 Ejemplo 3 - hsa-miR-423-5p_2_GT:\n")
example3 <- df_main %>%
  filter(feature == "hsa-miR-423-5p_2_GT") %>%
  select(feature, all_of(first_5_samples))
