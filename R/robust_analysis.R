# Robust Analysis Script for ALS miRNA Oxidation Research
# This script provides a comprehensive analysis with proper error handling
...
#' Run robust analysis with comprehensive G>T analysis
#'
...
#' @export
run_robust_analysis <- function(config_file = "config.yaml", output_dir = "outputs") {
  cat("=== Starting Robust ALS miRNA Oxidation Analysis ===\n")
  ...
  seed_summary <- seed_mutations %>%
    group_by(miRNA, in_seed) %>%
    summarise(
      total_gt_count = sum(select(., all_of(sample_cols)), na.rm = TRUE),
      .groups = "drop"
    ) %>%
    pivot_wider(names_from = in_seed, values_from = total_gt_count, values_fill = 0) %>%
    mutate(
      seed_gt = `TRUE`,
      non_seed_gt = `FALSE`,
      total_gt = seed_gt + non_seed_gt,
      seed_ratio = ifelse(total_gt > 0, seed_gt / total_gt, 0)
    )
  
  p_seed_analysis <- ggplot(seed_summary, aes(x = seed_ratio)) +
    geom_histogram(bins = 30, fill = "steelblue", alpha = 0.7) +
    labs(
      title = "Distribution of G>T Mutations in Seed Regions",
      x = "Proportion of G>T mutations in seed region (positions 2-8)",
      y = "Number of miRNAs"
    ) +
    theme_minimal()
  ...
  # Save plots
  ggsave(file.path(output_dir, "figures", "sample_distribution_robust.png"), p_sample_dist, 
         width = 10, height = 8, dpi = 300)
  ggsave(file.path(output_dir, "figures", "total_counts_distribution_robust.png"), p_counts_dist, 
         width = 10, height = 8, dpi = 300)
  ggsave(file.path(output_dir, "figures", "snv_distribution_robust.png"), p_snv_dist, 
         width = 10, height = 8, dpi = 300)
  ggsave(file.path(output_dir, "figures", "mutation_types_robust.png"), p_mutation_types, 
         width = 10, height = 8, dpi = 300)
  ggsave(file.path(output_dir, "figures", "seed_analysis_robust.png"), p_seed_analysis, 
         width = 10, height = 8, dpi = 300)
  
  if (!is.null(p_gt_mutations)) {
    ggsave(file.path(output_dir, "figures", "gt_mutations_robust.png"), p_gt_mutations, 
           width = 10, height = 8, dpi = 300)
    ggsave(file.path(output_dir, "figures", "gt_rates_robust.png"), p_gt_rates, 
           width = 10, height = 8, dpi = 300)
  }
  
  # Save data tables
  write.csv(group_stats, file.path(output_dir, "tables", "group_statistics_robust.csv"), row.names = FALSE)
  write.csv(mutation_type_counts, file.path(output_dir, "tables", "mutation_type_counts_robust.csv"), row.names = FALSE)
  write.csv(seed_summary, file.path(output_dir, "tables", "seed_analysis_robust.csv"), row.names = FALSE)
  
  if (!is.null(gt_analysis)) {
    write.csv(gt_analysis$gt_summary, file.path(output_dir, "tables", "gt_mutation_summary_robust.csv"), row.names = FALSE)
  }
...
if (interactive()) {
  cat("Running Robust ALS miRNA Oxidation Analysis...\n")
