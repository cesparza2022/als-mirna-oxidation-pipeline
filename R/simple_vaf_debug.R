...
# Simple debug script to understand VAF filter
library(dplyr)
library(readr)
...
# Take first SNV for detailed analysis
cat("\n🔍 ANALYZING FIRST SNV STEP BY STEP:\n")
cat(paste(rep("=", 60), collapse = ""), "\n")

i <- 1
cat("\n📋 SNV 1:", df[i, "miRNA name"][[1]], "|", df[i, "pos:mut"][[1]], "\n")
...
# Calculate VAFs for all SNVs (sample first 1000 for speed)
sample_size <- min(1000, nrow(df))
all_vafs <- matrix(0, nrow = sample_size, ncol = length(snv_cols))
...
cat("📊 Overall Statistics (first", sample_size, "SNVs):\n")
