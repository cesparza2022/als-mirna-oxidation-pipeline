# =============================================================================
# ANÁLISIS CORREGIDO POSICIÓN 6 - G>T MUTATIONS
# =============================================================================
# Script para analizar mutaciones G>T en todas las posiciones disponibles
...
cat("🔬 ANÁLISIS CORREGIDO MUTACIONES G>T - TODAS LAS POSICIONES\n")
cat("==========================================================\n\n")
...
# --- 2. VERIFICAR MUTACIONES G>T DISPONIBLES ---
cat("🎯 2. VERIFICAR MUTACIONES G>T DISPONIBLES\n")
cat("=========================================\n")
...
if (nrow(gt_mutations) > 0) {
  ...
  # --- 3. ANÁLISIS DETALLADO DE LAS POSICIONES CON G>T ---
  cat("\n🔍 3. ANÁLISIS DETALLADO POSICIONES CON G>T\n")
  cat("===========================================\n")
  ...
  # Analizar todas las posiciones con G>T
  available_positions <- position_summary$position
  ...
  p1 <- ggplot(comparison_data, aes(x = factor(Position), y = VAF, fill = Group)) +
    ...
    labs(
      title = "VAF Comparison: G>T Mutations by Position",
  ...
  p2 <- ggplot(zscore_data, aes(x = factor(Position), y = Z_Score, fill = Significance)) +
    ...
    labs(
      title = "Z-Score Analysis: G>T Mutations by Position",
  ...
  p3 <- ggplot(position_summary, aes(x = factor(position), y = count)) +
    geom_col(fill = "#45B7D1", alpha = 0.8) +
    labs(
      title = "Distribution of G>T Mutations by Position",
      subtitle = "All Positions with G>T Mutations",
      x = "Position",
  ...
  # Guardar gráficas
  ggsave("outputs/final_paper_graphs/gt_mutations_vaf_comparison.pdf", p1, 
         width = 10, height = 6, dpi = 300)
  ggsave("outputs/final_paper_graphs/gt_mutations_zscore_analysis.pdf", p2, 
         width = 10, height = 6, dpi = 300)
  ggsave("outputs/final_paper_graphs/gt_mutations_distribution.pdf", p3, 
         width = 10, height = 6, dpi = 300)
  
  cat("✅ Gráficas guardadas:\n")
  cat("   - gt_mutations_vaf_comparison.pdf\n")
  cat("   - gt_mutations_zscore_analysis.pdf\n")
  cat("   - gt_mutations_distribution.pdf\n\n")
  
  # --- 5. RESUMEN FINAL ---
  cat("📋 5. RESUMEN FINAL MUTACIONES G>T\n")
  cat("==================================\n")
  ...
  cat("📊 TODAS LAS POSICIONES CON G>T:\n")
  ...
  cat("❌ NO se encontraron mutaciones G>T en el dataset\n")
  cat("   Verificando otros tipos de mutaciones...\n")
  
  # Verificar otros tipos de mutaciones
  all_mutations <- df_main %>%
    filter(pos.mut != "PM") %>%
    mutate(mutation = str_extract(pos.mut, "[A-Z]>[A-Z]$")) %>%
    filter(!is.na(mutation)) %>%
    count(mutation, sort = TRUE)
  
  cat("\n📊 TIPOS DE MUTACIONES DISPONIBLES:\n")
  print(all_mutations)
}
