...
validate_input <- function(input_file, 
  ...
  tryCatch({
    if (expected_format == "csv" || file_ext == "csv") {
      # Try reading as CSV
      data_preview <- readr::read_csv(input_file, 
                               n_max = 5,
                               show_col_types = FALSE,
                               progress = FALSE)
    } else {
      # Try reading as TSV
      data_preview <- readr::read_tsv(input_file,
                               n_max = 5,
                               show_col_types = FALSE,
                               progress = FALSE)
    }
  ...
  if (validate_data_types && !is.null(required_columns)) {
    ...
    if (file_size_mb < 50) {  # Only validate types if file < 50 MB
      full_data <- readr::read_csv(input_file, show_col_types = FALSE, progress = FALSE)
...
validate_step1_5_input <- function(input_file) {
  ...
  if (detected_format == "csv") {
    data_preview <- readr::read_csv(input_file, n_max = 5, show_col_types = FALSE)
  } else {
    data_preview <- readr::read_tsv(input_file, n_max = 5, show_col_types = FALSE)
  }
