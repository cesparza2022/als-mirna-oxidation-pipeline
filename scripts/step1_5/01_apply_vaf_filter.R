...
cat(sprintf("   ✅ Detected columns: '%s' and '%s'\n", mirna_col, posmut_col))
...
data_with_info <- data %>%
  mutate(
    ...
    pos_mut = .data[[posmut_col]]
  )

# Get ID columns (non-sample columns)
id_cols <- c(mirna_col, posmut_col, "miRNA", "pos_mut")
...
long_data <- data_with_info %>%
  ...
  pivot_wider(
    id_cols = c(!!sym(mirna_col), !!sym(posmut_col), miRNA, pos_mut, Sample_Base),
    names_from = is_total,
    values_from = Count,
    names_prefix = "Count_"
  ) %>%
...
data_filtered <- long_data_filtered %>%
  select(!!sym(mirna_col), !!sym(posmut_col), Sample = Sample_Base, SNV_Count = SNV_Count_Filtered) %>%
  pivot_wider(
    id_cols = c(!!sym(mirna_col), !!sym(posmut_col)),
    names_from = Sample,
    values_from = SNV_Count
  )
  # NOTE: Removed total columns merge - output now contains only SNV count columns (415 columns)
  # This avoids confusion in downstream steps that need to filter out total columns
...
if (length(snv_cols) > 0 && nrow(data) > 0) {
